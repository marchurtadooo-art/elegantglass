...
export default function ScanScreen() {
  ...
  const handleCode = async (raw: string) => {
    ...
    // Zone QR format generated by backend: GW-ZONE-XXXXXXXX
    if (/^GW-ZONE-/i.test(code)) {
      setProcessing(true);
      if (Platform.OS !== 'web') {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch(() => {});
      }
      try {
        const r = await api.get(`/warehouse/zones/by-qr/${encodeURIComponent(code)}`);
        const zoneNumber = r.data?.zone?.zone_number;
        if (zoneNumber != null) {
          router.replace({ pathname: '/warehouse/map', params: { zoneNumber: String(zoneNumber) } } as any);
        } else {
          Alert.alert('Zona no encontrada', 'No se ha podido localizar la zona escaneada.', [
